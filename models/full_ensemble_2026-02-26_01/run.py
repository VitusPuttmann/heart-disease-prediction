...
from src.ensemble import (
    evaluate_ensemble,
    fit_stacking_regression,
    predict_stacking_regression
)
...
if __name__ == "__main__":
    ...
    if RU_CONFIG["fit_model"]:
        ...
        oof_cols = [m for m in RU_CONFIG["models"]
        ...
        if ens_cfg.get("method") == "stacking_regression":
            ...
            y = train_data_prepared[PR_CONFIG["label_col"]]
    ...
    base_cols = [f"y_proba_{m}" for m in RU_CONFIG["models"] if f"y_proba_{m}" in output_df.columns]

    if method == "stacking_regression" and RU_CONFIG["fit_model"]:
        # Train meta-model on OOF predictions (aligned to training rows)
        oof_cols = [m for m in RU_CONFIG["models"]
            if (m in oof_pred_all.columns) and (f"y_proba_{m}" in output_df.columns)]
        X_oof = oof_pred_all[oof_cols]
        y = train_data_prepared[PR_CONFIG["label_col"]]
        ...
        # Predict stacked probabilities on test
        X_test = pd.DataFrame(
